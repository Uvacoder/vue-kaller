                        ...
                        <template v-slot:expanded-item="{ item }">
                            <div class="d-block d-md-flex">
                                <div :class="{'expandedtext': $vuetify.breakpoint.mdAndUp, 'pa-2': true}">
                                    ...
                                    <v-spacer></v-spacer>
                                    <v-btn color="warning" @click="photoResize(item.filename)">
                                        Resize
                                    </v-btn>
                                    <v-btn color="success" @click="photoHide(item.filename)">
                                        Hide
                                    </v-btn>
                                    <v-btn color="error" @click="photoDelete(item.filename)">
                                        Delete
                                    </v-btn>
                                </div>
                                <v-spacer></v-spacer>
    ...
    export default {
        ...
        methods: {
            db: async function () {
                ...
                this.text = await api.post({mode: 'db', auth: localStorage.auth}, 'reload');
                this.snackbar = true;
            ...
            resize: async function () {
                ...
                this.text = await api.post({mode: 'resize', auth: localStorage.auth}, 'reload');
                this.snackbar = true;
                this.snackcolor = 'success';
                console.log(this.text);
                this.loader = false;
            ...
            async photoDelete(filename) {
                this.text = await api.post({filename: filename, auth: localStorage.auth}, 'deletephoto');
                this.snackcolor = 'error';
                this.snackbar = true;
                this.loadStats();
                this.$store.dispatch('getPhotos');
            ...
            async photoHide(filename) {
                this.text = await api.post({filename: filename, mode: 'hide', auth: localStorage.auth}, 'hidephoto');
            ...
            async photoResize(filename) {
                this.text = await api.post({filename: filename, auth: localStorage.auth}, 'resizephoto');
        ...
        data() {
            return {
                ...
                lensLabels: ['DJI Phantom 3', 'Canon EF 70-200 f4L', 'Sony FE 16-35 F4','Minolta 45 F2'],
            };
        },
        watch: {
            loader() {
